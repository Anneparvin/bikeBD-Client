...
import { useDeleteProductMutation, useGetAllProductsQuery } from "@/redux/features/bikeProduct/bikeProductApi";
...
const ProductsManagment = () => {
    ...
    const { data: bikeData, isLoading, isError } = useGetAllProductsQuery([
        { name: "page", value: currentPage },
        { name: "limit", value: 6 },
    ]);
    console.log(bikeData);

    const bikes = bikeData?.data?.product;
    const totalBike = bikeData?.data?.result?.total;
    const totalPage = bikeData?.data?.result?.page;
    const limit = bikeData?.data?.result?.limit;
    ...
    if (!isLoading &&!bikes || !Array.isArray(bikes)) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    await deleteProduct(bi._id).unwrap();
        ...
        <div className="">
            <section className="container lg:px-16 md:px-10 px-4 mx-auto min-h-screen ">
                <div className="flex items-center justify-center gap-3 ">
                    <h2 className=" font-medium text-gray-800 dark:text-white text-3xl">All Bike Products</h2>
                    <span className="px-2 text-lg border-blue-500 border-2  rounded-full "> {totalBike}</span>
                </div>
                            ...
                            <div className="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
                                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                                        {
                                            bikes.map((bi: TBike, index: number) => {
